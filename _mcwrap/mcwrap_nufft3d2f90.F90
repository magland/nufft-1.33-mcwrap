...
!-----Setup the set inputs
        ...
        !ms
        input_ms=int(mcwrap_size(prhs(6),1))
        !mt
        input_mt=int(mcwrap_size(prhs(6),2))
        !mu
        input_mu=int(mcwrap_size(prhs(6),3))
...
!-----Setup the inputs
        ...
        DEALLOCATE(dims2)
        p_input_fk_re=mxGetPr(prhs(6));
        p_input_fk_im=mxGetPi(prhs(6));
        ALLOCATE(input_fk((input_ms)*(input_mt)*(input_mu)*2))
        ALLOCATE(input_fk_re((input_ms)*(input_mt)*(input_mu)))
        ALLOCATE(input_fk_im((input_ms)*(input_mt)*(input_mu)))
        call mxCopyPtrToReal8(p_input_fk_re,input_fk_re,int((input_ms)*(input_mt)*(input_mu)))
        if (p_input_fk_im .NE. 0) then
            call mxCopyPtrToReal8(p_input_fk_im,input_fk_im,int((input_ms)*(input_mt)*(input_mu)))
        end if
