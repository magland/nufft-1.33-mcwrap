...
!-----Setup the set inputs
        ...
        !ms
        input_ms=int(mcwrap_size(prhs(5),1))
        !mt
        input_mt=int(mcwrap_size(prhs(5),2))
...
!-----Setup the inputs
        ...
        DEALLOCATE(dims2)
        p_input_fk_re=mxGetPr(prhs(5));
        p_input_fk_im=mxGetPi(prhs(5));
        ALLOCATE(input_fk((input_ms)*(input_mt)*2))
        ALLOCATE(input_fk_re((input_ms)*(input_mt)))
        ALLOCATE(input_fk_im((input_ms)*(input_mt)))
        call mxCopyPtrToReal8(p_input_fk_re,input_fk_re,int((input_ms)*(input_mt)))
        if (p_input_fk_im .NE. 0) then
            call mxCopyPtrToReal8(p_input_fk_im,input_fk_im,int((input_ms)*(input_mt)))
        end if
        do ii=1,(input_ms)*(input_mt)
            input_fk(ii*2)=input_fk_re(ii)
            if (p_input_fk_im .NE. 0) then
            input_fk(ii*2+1)=input_fk_im(ii)
            else
            input_fk(ii*2+1)=0    
            end if
        end do
...
!-----Set the outputs
        ...
        if (1 .LE. nlhs) then
            do ii=1,(input_nj)*(1)
                output_cj_re(ii)=output_cj(ii*2)
                output_cj_im(ii)=output_cj(ii*2+1)
            end do
