...
!     Declare inputs:
        ...
        Integer input_sk
        real*8 input_sk_double
        !tk
        mwPointer p_input_tk
        Integer input_tk
        real*8 input_tk_double
        !uk
        mwPointer p_input_uk
        Integer input_uk
        real*8 input_uk_double
...
!-----Setup the set inputs
        ...
        !nk
        input_nk=int(mcwrap_size(prhs(7),1))
...
!-----Setup the inputs
        ...
        call mxCopyPtrToReal8(p_input_sk,input_sk_double,1)
        input_sk=int(input_sk_double)
        !tk
        p_input_tk=mxGetPr(prhs(8));
        call mxCopyPtrToReal8(p_input_tk,input_tk_double,1)
        input_tk=int(input_tk_double)
        !uk
        p_input_uk=mxGetPr(prhs(9));
        call mxCopyPtrToReal8(p_input_uk,input_uk_double,1)
        input_uk=int(input_uk_double)
...
!-----Setup the outputs
        ...
        do ii=1,2
            if ((dims2(ii) .lt. 1) .or. (dims2(ii) .gt. 10000000000.0)) then
              call mexErrMsgTxt ('Bad array size for my taste: 1,input_nk') 
            end if
        end do
        DEALLOCATE(dims2)
        
            plhs(1)=mxCreateNumericArray(2,(/ 1,input_nk /),mxClassIDFromClassName('double'),1)
            p_output_fk_re=mxGetPr(plhs(1))
            p_output_fk_im=mxGetPi(plhs(1))
        end if
         ALLOCATE(output_fk(int((1)*(input_nk))*2))
        ALLOCATE(output_fk_re(int((1)*(input_nk))))
        ALLOCATE(output_fk_im(int((1)*(input_nk))))
        !ier
        if (2 .LE. nlhs) then
        if ((2 .lt. 1) .or. (2 .gt. 20)) then
          call mexErrMsgTxt ('Bad number of dimensions for my taste: 2') 
...
!-----Set the outputs
        ...
        if (1 .LE. nlhs) then
            ...
            call mxCopyReal8ToPtr(output_fk_re,p_output_fk_re,int((1)*(input_nk)))
            call mxCopyReal8ToPtr(output_fk_im,p_output_fk_im,int((1)*(input_nk)))
        end if
