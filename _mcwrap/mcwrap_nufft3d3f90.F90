...
!-----Setup the inputs
        ...
        call mxCopyPtrToInteger4(mxGetDimensions(prhs(8)),dims,numdims)
        ALLOCATE(dims2(2))
        dims2=(/ input_nk,1 /)
        do ii=1,2
          if (ii .le. numdims) then
        ...
        if (numdims .gt. 2) then
          call mexErrMsgTxt('Incorrect number of dimensions in input: uk')
        end if
        call mxCopyPtrToInteger4(mxGetDimensions(prhs(9)),dims,numdims)
        ALLOCATE(dims2(2))
        dims2=(/ input_nk,1 /)
        do ii=1,2
          if (ii .le. numdims) then
