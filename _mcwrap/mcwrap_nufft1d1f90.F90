...
!-----Setup the inputs
        ...
        DEALLOCATE(dims2)
        p_input_cj_re=mxGetPr(prhs(2));
        p_input_cj_im=mxGetPi(prhs(2));
...
!-----Setup the outputs
        ...
        do ii=1,2
            if ((dims2(ii) .lt. 1) .or. (dims2(ii) .gt. 10000000000.0)) then
              call mexErrMsgTxt ('Bad array size for my taste: input_ms,1') 
            end if
        end do
        DEALLOCATE(dims2)
        
            plhs(1)=mxCreateNumericArray(2,(/ input_ms,1 /),mxClassIDFromClassName('double'),1)
            p_output_fk_re=mxGetPr(plhs(1))
            p_output_fk_im=mxGetPi(plhs(1))
        end if
         ALLOCATE(output_fk(int((input_ms)*(1))*2))
        ALLOCATE(output_fk_re(int((input_ms)*(1))))
        ALLOCATE(output_fk_im(int((input_ms)*(1))))
        !ier
        if (2 .LE. nlhs) then
        if ((2 .lt. 1) .or. (2 .gt. 20)) then
          call mexErrMsgTxt ('Bad number of dimensions for my taste: 2') 
...
!-----Set the outputs
        ...
        if (1 .LE. nlhs) then
            ...
            call mxCopyReal8ToPtr(output_fk_re,p_output_fk_re,int((input_ms)*(1)))
            call mxCopyReal8ToPtr(output_fk_im,p_output_fk_im,int((input_ms)*(1)))
        end if
